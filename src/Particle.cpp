...
//Used in pause when clicked s
void Particle::setPause(particlePause newPause){
	pause = newPause;
}

void Particle::setColor(particleColor newColor){
	color = newColor;
}
...
void Particle::setMagnify(double mag){
	this -> magnify = mag;
}
...
void Particle::draw(){
	...
	//TODO: ask if should use else if 
	else{
		ofSetColor(0,255,0);
	}
	if(color == CUSTOM){
		ofNoFill();
		ofDrawCircle(pos.x, pos.y, scale * 4.0 * magnify);
	}
	else {
		ofFill();
		ofDrawCircle(pos.x, pos.y, scale * 4.0 * magnify);
